...
// usage: node scripts/import-chapter.mjs "ch1-80-mcqs.docx" "Chapter 1"
const FILE = process.argv[2];
const CHAPTER = process.argv[3];

if (!FILE || !CHAPTER) {
  console.error('Usage: node scripts/import-chapter.mjs "file.docx" "Chapter 1"');
  process.exit(1);
}
...
async function run() {
  ...
  console.log(`Parsed ${questions.length}, valid ${valid.length}, chapter "${CHAPTER}"`);

  let saved = 0;
  for (const x of valid) {
    await prisma.question.create({
      data: {
        topic: CHAPTER, subject: "", chapter: CHAPTER,
  ...
  console.log(`✅ Saved ${saved} questions under chapter "${CHAPTER}".`);
